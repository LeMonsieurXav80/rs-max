<?php

namespace App\Http\Controllers;

use App\Models\PostPlatform;
use App\Models\SocialAccount;
use Carbon\Carbon;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\DB;
use Illuminate\View\View;

class StatsController extends Controller
{
    /**
     * Display the statistics dashboard with filters.
     */
    public function dashboard(Request $request): View
    {
        $user = $request->user();

        // Get user's social accounts for filter
        if ($user->is_admin) {
            $socialAccounts = SocialAccount::with('platform')->orderBy('name')->get();
        } else {
            $socialAccounts = $user->socialAccounts()->with('platform')->orderBy('name')->get();
        }

        // Get filters
        $selectedAccounts = $request->input('accounts', []);
        $period = $request->input('period', '30'); // days
        $startDate = $request->input('start_date');
        $endDate = $request->input('end_date');

        // Build query
        $query = PostPlatform::with(['post', 'platform', 'socialAccount'])
            ->where('status', 'published')
            ->whereNotNull('external_id')
            ->whereNotNull('metrics');

        // Filter by accounts
        if (! empty($selectedAccounts)) {
            $query->whereIn('social_account_id', $selectedAccounts);
        } else {
            // If no accounts selected, use user's accounts
            if (! $user->is_admin) {
                $accountIds = $user->socialAccounts()->pluck('social_accounts.id');
                $query->whereIn('social_account_id', $accountIds);
            }
        }

        // Filter by date range
        if ($startDate && $endDate) {
            $query->whereBetween('published_at', [$startDate, $endDate]);
        } elseif ($period !== 'all') {
            $query->where('published_at', '>=', now()->subDays((int) $period));
        }

        $postPlatforms = $query->orderBy('published_at', 'desc')->get();

        // Calculate aggregate stats
        $stats = $this->calculateAggregateStats($postPlatforms);

        // Get top posts (by engagement)
        $topPosts = $this->getTopPosts($postPlatforms, 10);

        // Get stats by platform
        $statsByPlatform = $this->getStatsByPlatform($postPlatforms);

        // Get stats by account
        $statsByAccount = $this->getStatsByAccount($postPlatforms);

        // Get timeline data for chart (daily aggregates)
        $timeline = $this->getTimelineData($postPlatforms);

        return view('stats.dashboard', compact(
            'socialAccounts',
            'selectedAccounts',
            'period',
            'startDate',
            'endDate',
            'stats',
            'topPosts',
            'statsByPlatform',
            'statsByAccount',
            'timeline',
            'postPlatforms'
        ));
    }

    /**
     * Calculate aggregate statistics.
     */
    private function calculateAggregateStats($postPlatforms): array
    {
        $totalViews = 0;
        $totalLikes = 0;
        $totalComments = 0;
        $totalShares = 0;
        $postsCount = $postPlatforms->count();

        foreach ($postPlatforms as $pp) {
            $metrics = $pp->metrics ?? [];
            $totalViews += $metrics['views'] ?? 0;
            $totalLikes += $metrics['likes'] ?? 0;
            $totalComments += $metrics['comments'] ?? 0;
            $totalShares += $metrics['shares'] ?? 0;
        }

        // Calculate engagement rate (likes + comments + shares) / views
        $totalEngagement = $totalLikes + $totalComments + $totalShares;
        $engagementRate = $totalViews > 0 ? ($totalEngagement / $totalViews) * 100 : 0;

        return [
            'posts_count' => $postsCount,
            'total_views' => $totalViews,
            'total_likes' => $totalLikes,
            'total_comments' => $totalComments,
            'total_shares' => $totalShares,
            'total_engagement' => $totalEngagement,
            'engagement_rate' => round($engagementRate, 2),
            'avg_views_per_post' => $postsCount > 0 ? round($totalViews / $postsCount) : 0,
            'avg_likes_per_post' => $postsCount > 0 ? round($totalLikes / $postsCount) : 0,
        ];
    }

    /**
     * Get top posts by engagement.
     */
    private function getTopPosts($postPlatforms, int $limit = 10): array
    {
        // Group by post and sum metrics
        $postStats = [];

        foreach ($postPlatforms as $pp) {
            $postId = $pp->post_id;
            $metrics = $pp->metrics ?? [];

            if (! isset($postStats[$postId])) {
                $postStats[$postId] = [
                    'post' => $pp->post,
                    'views' => 0,
                    'likes' => 0,
                    'comments' => 0,
                    'shares' => 0,
                    'engagement' => 0,
                ];
            }

            $postStats[$postId]['views'] += $metrics['views'] ?? 0;
            $postStats[$postId]['likes'] += $metrics['likes'] ?? 0;
            $postStats[$postId]['comments'] += $metrics['comments'] ?? 0;
            $postStats[$postId]['shares'] += $metrics['shares'] ?? 0;
        }

        // Calculate engagement for each post
        foreach ($postStats as &$stat) {
            $stat['engagement'] = $stat['likes'] + $stat['comments'] + $stat['shares'];
        }

        // Sort by engagement
        usort($postStats, fn ($a, $b) => $b['engagement'] <=> $a['engagement']);

        return array_slice($postStats, 0, $limit);
    }

    /**
     * Get stats grouped by platform.
     */
    private function getStatsByPlatform($postPlatforms): array
    {
        $platformStats = [];

        foreach ($postPlatforms as $pp) {
            $slug = $pp->platform->slug;
            $metrics = $pp->metrics ?? [];

            if (! isset($platformStats[$slug])) {
                $platformStats[$slug] = [
                    'name' => $pp->platform->name,
                    'slug' => $slug,
                    'count' => 0,
                    'views' => 0,
                    'likes' => 0,
                    'comments' => 0,
                    'shares' => 0,
                ];
            }

            $platformStats[$slug]['count']++;
            $platformStats[$slug]['views'] += $metrics['views'] ?? 0;
            $platformStats[$slug]['likes'] += $metrics['likes'] ?? 0;
            $platformStats[$slug]['comments'] += $metrics['comments'] ?? 0;
            $platformStats[$slug]['shares'] += $metrics['shares'] ?? 0;
        }

        return array_values($platformStats);
    }

    /**
     * Get stats grouped by account.
     */
    private function getStatsByAccount($postPlatforms): array
    {
        $accountStats = [];

        foreach ($postPlatforms as $pp) {
            $accountId = $pp->social_account_id;
            $metrics = $pp->metrics ?? [];

            if (! isset($accountStats[$accountId])) {
                $accountStats[$accountId] = [
                    'account' => $pp->socialAccount,
                    'platform' => $pp->platform,
                    'count' => 0,
                    'views' => 0,
                    'likes' => 0,
                    'comments' => 0,
                    'shares' => 0,
                ];
            }

            $accountStats[$accountId]['count']++;
            $accountStats[$accountId]['views'] += $metrics['views'] ?? 0;
            $accountStats[$accountId]['likes'] += $metrics['likes'] ?? 0;
            $accountStats[$accountId]['comments'] += $metrics['comments'] ?? 0;
            $accountStats[$accountId]['shares'] += $metrics['shares'] ?? 0;
        }

        // Sort by total engagement
        usort($accountStats, function ($a, $b) {
            $engagementA = $a['likes'] + $a['comments'] + $a['shares'];
            $engagementB = $b['likes'] + $b['comments'] + $b['shares'];

            return $engagementB <=> $engagementA;
        });

        return $accountStats;
    }

    /**
     * Get timeline data for charts (daily aggregates).
     */
    private function getTimelineData($postPlatforms): array
    {
        $timeline = [];

        foreach ($postPlatforms as $pp) {
            $date = $pp->published_at->format('Y-m-d');
            $metrics = $pp->metrics ?? [];

            if (! isset($timeline[$date])) {
                $timeline[$date] = [
                    'date' => $date,
                    'posts' => 0,
                    'views' => 0,
                    'likes' => 0,
                    'comments' => 0,
                    'shares' => 0,
                ];
            }

            $timeline[$date]['posts']++;
            $timeline[$date]['views'] += $metrics['views'] ?? 0;
            $timeline[$date]['likes'] += $metrics['likes'] ?? 0;
            $timeline[$date]['comments'] += $metrics['comments'] ?? 0;
            $timeline[$date]['shares'] += $metrics['shares'] ?? 0;
        }

        // Sort by date
        ksort($timeline);

        return array_values($timeline);
    }
}
