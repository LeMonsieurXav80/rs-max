                    ...
                    <div class="divide-y divide-gray-50">
                        @foreach($appAccounts as $account)
                            <div class="px-6 py-4" x-data="{
                                ...
                                <div class="flex items-center justify-between gap-3">
                                    ...
                                    <div class="flex items-center gap-2">
                                        ...
                                        <button
                                            ...
                                            :class="editing ? 'text-indigo-600 bg-indigo-50' : ''"
                                            title="Modifier"
                                        >
                                ...
                                <div x-show="editing" x-collapse x-cloak class="mt-4 pt-4 border-t border-gray-100">
                                    <form action="{{ route('platforms.twitter.updateAccount', $account) }}" method="POST" class="space-y-3">
                                        @csrf
                                        @method('PUT')
                                        <div>
                                            <label class="block text-xs font-medium text-gray-500 mb-1">Nom du compte</label>
                                        ...
                                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                            <div>
                                                ...
                                                <input type="text" name="api_key" value="{{ $account->credentials['api_key'] ?? '' }}" required
                                                    class="w-full rounded-xl border-gray-300 text-sm shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                            </div>
                                            <div>
                                                <label class="block text-xs font-medium text-gray-500 mb-1">API Secret</label>
                                                <input type="password" name="api_secret" value="{{ $account->credentials['api_secret'] ?? '' }}" required
                                                    class="w-full rounded-xl border-gray-300 text-sm shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                            </div>
                                            <div>
                                                <label class="block text-xs font-medium text-gray-500 mb-1">Access Token</label>
                                                <input type="text" name="access_token" value="{{ $account->credentials['access_token'] ?? '' }}" required
                                                    class="w-full rounded-xl border-gray-300 text-sm shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                            </div>
                                            <div>
                                                <label class="block text-xs font-medium text-gray-500 mb-1">Access Token Secret</label>
                                                <input type="password" name="access_token_secret" value="{{ $account->credentials['access_token_secret'] ?? '' }}" required
                                                    class="w-full rounded-xl border-gray-300 text-sm shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                            </div>
                                        </div>
                                        <div class="flex justify-end gap-2 pt-1">
                                            <button type="button" @click="editing = false"
                                                class="px-4 py-2 text-xs font-medium text-gray-600 bg-white border border-gray-200 rounded-xl hover:bg-gray-50 transition-colors">
                                                Annuler
                                            </button>
                                            <button type="submit"
                                                class="px-4 py-2 text-xs font-medium text-white bg-gray-900 rounded-xl hover:bg-gray-800 transition-colors shadow-sm">
                                                Enregistrer
                                            </button>
                                        </div>
                                    </form>
