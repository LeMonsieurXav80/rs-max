...
@section('title', 'Prévisualiser — ' . $rssFeed->name)
...
<div x-data="rssPreview()" class="max-w-5xl space-y-6">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
        <div class="flex items-center gap-4 mb-4">
            ...
            <div>
                <h2 class="text-lg font-bold text-gray-900">{{ $rssFeed->name }}</h2>
                <p class="text-sm text-gray-500">{{ $rssFeed->url }}</p>
            </div>
        </div>
        <div class="flex flex-wrap gap-4 text-sm text-gray-600">
            ...
            <span class="inline-flex items-center gap-1.5 px-3 py-1 bg-gray-50 rounded-lg">
                ...
                {{ $frequencyLabels[$rssFeed->schedule_frequency] ?? $rssFeed->schedule_frequency }} &middot; {{ $rssFeed->schedule_time }}
            </span>
            <span class="inline-flex items-center gap-1.5 px-3 py-1 bg-gray-50 rounded-lg">
                <svg class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M13.19 8.688a4.5 4.5 0 0 1 1.242 7.244l-4.5 4.5a4.5 4.5 0 0 1-6.364-6.364l1.757-1.757m13.35-.622 1.757-1.757a4.5 4.5 0 0 0-6.364-6.364l-4.5 4.5a4.5 4.5 0 0 0 1.242 7.244" />
                </svg>
                {{ $rssFeed->socialAccounts->count() }} compte(s) lié(s)
            </span>
    ...
    <div x-show="publications.length > 0" x-transition class="space-y-4">
        <template x-for="(pub, pubIndex) in publications" :key="pub.rss_item_id">
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                ...
                <div class="p-5" x-data="{ activeTab: Object.keys(pub.platform_contents)[0] }">
                    ...
                    <div class="flex gap-1 border-b border-gray-200 mb-4 overflow-x-auto" x-show="Object.keys(pub.platform_contents).length > 1">
                        <template x-for="accountId in Object.keys(pub.platform_contents)" :key="'tab-' + pub.rss_item_id + '-' + accountId">
                    ...
                    {{-- Tab panels --}}
                    <template x-for="accountId in Object.keys(pub.platform_contents)" :key="'panel-' + pub.rss_item_id + '-' + accountId">
...
function rssPreview() {
    return {
        ...
        confirmMessage: '',
        confirmError: '',
        frequency: @json($rssFeed->schedule_frequency),
        scheduleTime: @json($rssFeed->schedule_time),
        ...
        async generate() {
            ...
            try {
                // Step 1: Select articles (fast, no AI)
                const resp = await fetch('{{ route("rss-feeds.generatePreview", $rssFeed) }}', {
                    method: 'POST',
                    headers: {
                ...
                // Show articles immediately (empty content)
                ...
                // Step 2: Generate content item by item via regenerateItem
                ...
                for (let i = 0; i < total; i++) {
                    ...
                    try {
                        const genResp = await fetch('{{ route("rss-feeds.regenerateItem", $rssFeed) }}', {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json',
                                'X-CSRF-TOKEN': document.querySelector('meta[name=csrf-token]').content,
                            ...
                            body: JSON.stringify({ rss_item_id: pub.rss_item_id }),
                        });
                        ...
                        console.error(`Generation failed for item ${pub.rss_item_id}:`, e);
                    }
        ...
        async regenerate(pubIndex) {
            ...
            try {
                const resp = await fetch('{{ route("rss-feeds.regenerateItem", $rssFeed) }}', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': document.querySelector('meta[name=csrf-token]').content,
                ...
                if (data.platform_contents) {
                    // Update contents while preserving the structure
                ...
                // Silent fail, content remains as-is
            }
        ...
        async confirm() {
            ...
            // Build the payload with potentially edited content
            const payload = this.publications.map(pub => ({
                rss_item_id: pub.rss_item_id,
            ...
            try {
                const resp = await fetch('{{ route("rss-feeds.confirmPublications", $rssFeed) }}', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': document.querySelector('meta[name=csrf-token]').content,
                ...
                if (data.success) {
                    ...
                    // Clear publications after successful confirmation
                    setTimeout(() => {
                        this.publications = [];
                    }, 3000);
                } else if (data.error) {
