...
# Reset orphaned prospect targets and clear stale bot cache keys from previous container
php artisan tinker --execute="
App\Models\BotTargetAccount::where('status', 'running')->update(['status' => 'pending']);
\DB::table('cache')->where('key', 'like', '%bot_running%')->delete();
\DB::table('cache')->where('key', 'like', '%bot_stop%')->delete();
" 2>/dev/null || true
