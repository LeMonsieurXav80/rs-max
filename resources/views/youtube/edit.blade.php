...
@section('title', 'Modifier chaîne YouTube')
...
@section('content')
    ...
    <form method="POST" action="{{ route('youtube-channels.update', $ytSource) }}" class="max-w-3xl"
          x-data="{
              testing: false,
              testResult: null,
              testError: null,
              channelId: @js($ytSource->channel_id ?? ''),
              channelName: @js($ytSource->channel_name ?? ''),
              thumbnailUrl: @js($ytSource->thumbnail_url ?? ''),
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 space-y-6">
            ...
            <div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    <div>
                        ...
                        <input type="text" name="name" id="name" value="{{ old('name', $ytSource->name) }}" required
                    ...
                    <div>
                        ...
                        <input type="text" name="description" id="description" value="{{ old('description', $ytSource->description) }}"
                               class="w-full rounded-xl border-gray-300 shadow-sm focus:border-red-500 focus:ring-red-500 text-sm">
                ...
                <div class="mt-4">
                    ...
                    <input type="url" name="channel_url" id="channel_url" value="{{ old('channel_url', $ytSource->channel_url) }}" required
                           placeholder="https://www.youtube.com/@username ou https://www.youtube.com/channel/UC..."
                           class="w-full rounded-xl border-gray-300 shadow-sm focus:border-red-500 focus:ring-red-500 text-sm">
                    @error('channel_url') <p class="mt-1 text-xs text-red-600">{{ $message }}</p> @enderror
                </div>

                {{-- Hidden fields for channel data --}}
                <input type="hidden" name="channel_id" :value="channelId">
                <input type="hidden" name="channel_name" :value="channelName">
                <input type="hidden" name="thumbnail_url" :value="thumbnailUrl">

                {{-- Current channel info --}}
                <div x-show="channelId" class="mt-4 p-4 bg-gray-50 rounded-xl">
                    <p class="text-xs font-medium text-gray-600 mb-3">Chaîne actuelle</p>
                    <div class="flex items-center gap-3">
                        <img x-show="thumbnailUrl" :src="thumbnailUrl" class="w-12 h-12 rounded-full object-cover" alt="">
                        <div>
                            <p class="text-sm font-medium text-gray-900" x-text="channelName"></p>
                            <p class="text-xs text-gray-500" x-text="channelId"></p>
                        </div>
                    </div>
                </div>
                ...
                <div class="mt-4">
                    ...
                    <div x-show="testResult" x-transition class="mt-3 rounded-xl bg-green-50 border border-green-200 p-4">
                        <p class="text-sm text-green-700 font-medium">Connexion réussie <span x-text="testResult?.channel_name ? '(' + testResult.channel_name + ')' : ''"></span></p>
                        <p class="text-xs text-green-600 mt-1" x-text="'ID: ' + (testResult?.channel_id || '')"></p>
                    </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    <div>
                        ...
                        <select name="schedule_frequency" id="schedule_frequency"
                                class="w-full rounded-xl border-gray-300 shadow-sm focus:border-red-500 focus:ring-red-500 text-sm">
                            <option value="daily" {{ old('schedule_frequency', $ytSource->schedule_frequency) === 'daily' ? 'selected' : '' }}>Quotidien</option>
                            <option value="twice_weekly" {{ old('schedule_frequency', $ytSource->schedule_frequency) === 'twice_weekly' ? 'selected' : '' }}>2x par semaine</option>
                            <option value="weekly" {{ old('schedule_frequency', $ytSource->schedule_frequency) === 'weekly' ? 'selected' : '' }}>Hebdomadaire</option>
                            <option value="biweekly" {{ old('schedule_frequency', $ytSource->schedule_frequency) === 'biweekly' ? 'selected' : '' }}>Tous les 15 jours</option>
                            <option value="monthly" {{ old('schedule_frequency', $ytSource->schedule_frequency) === 'monthly' ? 'selected' : '' }}>Mensuel</option>
                        </select>
                    ...
                    <div>
                        ...
                        <input type="time" name="schedule_time" id="schedule_time" value="{{ old('schedule_time', $ytSource->schedule_time ?? '10:00') }}"
                               class="w-full rounded-xl border-gray-300 shadow-sm focus:border-red-500 focus:ring-red-500 text-sm">
            ...
            <div class="flex items-center gap-3">
                ...
                <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $ytSource->is_active) ? 'checked' : '' }}
                       class="rounded border-gray-300 text-red-600 shadow-sm focus:ring-red-500">
