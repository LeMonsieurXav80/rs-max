...
@section('content')
    <form method="POST" action="{{ route('posts.store') }}" class="max-w-4xl space-y-6" x-data="{
            ...
            @if($accounts->count())
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    @php
                        ...
                        $platformOrder = ['facebook', 'instagram', 'threads', 'twitter', 'bluesky', 'telegram', 'youtube', 'linkedin', 'pinterest'];
                    @endphp
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 lg:p-8">
            ...
            <div class="space-y-6">
                {{-- Contenu par défaut (français) --}}
                <div>
                    <label for="content_fr" class="block text-sm font-medium text-gray-700 mb-2">
                        Contenu par défaut <span class="text-red-500">*</span>
                    </label>
                    <textarea
                        id="content_fr"
                        name="content_fr"
                        ...
                        required
                        class="w-full rounded-xl border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm placeholder-gray-400 transition-colors"
                        placeholder="Rédigez votre publication en français (sera utilisé si aucun contenu plateforme n'est défini)..."
                ...
                @include('posts._platform-tabs', [
                    'initialPlatformContents' => old('platform_contents', []),
                    'articleTitle' => old('article_title', ''),
                ])
