...
@section('actions')
    <div class="flex items-center gap-3" x-data="{ publishingAll: false, publishResult: null }">
        ...
        @if($post->status === 'published')
            <a href="{{ route('posts.reshare.form', $post) }}"
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            @php
                ...
                if ($hasStats) {
                    ...
                    $totalBookmarks = $platformsWithMetrics->sum(fn($pp) => $pp->metrics['bookmarks'] ?? $pp->metrics['saved'] ?? 0);
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 lg:p-8">
                    ...
                    @if($hasStats)
                        ...
                        <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4 mb-6">
                            ...
                            <div>
                                <p class="text-2xl font-bold text-indigo-600">{{ number_format($totalBookmarks, 0, ',', ' ') }}</p>
                                <p class="text-xs text-gray-500 mt-1">Enregistrements</p>
                            </div>
                            @if($totalViews > 0)
                                <div>
                        ...
                        <div class="space-y-3">
                            @foreach($platformsWithMetrics as $pp)
                                @php
                                    $m = $pp->metrics;
                                    $ppViews = isset($m['views']) ? (int) $m['views'] : null;
                                ...
                                <div class="flex items-center gap-3 py-2">
                                    ...
                                    <div class="flex items-center gap-x-4 text-xs flex-1">
                                        <span class="inline-flex items-center gap-1 text-gray-600" title="Vues">
                                            ...
                                            <span class="font-medium text-gray-900">{{ $ppViews !== null ? number_format($ppViews, 0, ',', ' ') : '-' }}</span>
                                        </span>
                                        ...
                                        <span class="inline-flex items-center gap-1 text-gray-600" title="Enregistrements">
                                            <svg class="w-3.5 h-3.5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5">
                        ...
                        <div class="px-6 lg:px-8 py-4" x-data="{ publishing: false, resetting: false, result: null, error: null }">
                            <div class="flex items-center justify-between gap-4">
                                ...
                                <div class="flex items-center gap-2 flex-shrink-0">
                                    {{-- Sponsoriser : seulement une publication Meta reellement publiee.
                                         Le post d'origine est promu avec ses likes, rien n'est republie. --}}
                                    @if($pp->status === 'published' && $pp->external_id
                                        && in_array($pp->platform?->slug, ['facebook', 'instagram'])
                                        && auth()->user()->isManager())
                                        <a href="{{ route('ads.boost.form', $pp) }}"
                                           class="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-lg bg-blue-50 text-blue-700 border border-blue-200 hover:bg-blue-100 transition-colors">
