...
@section('content')
    <form method="POST" action="{{ route('threads.store') }}" class="max-w-4xl space-y-6" x-data="threadForm()" @submit="handleSubmit($event)">
                        ...
                        <div class="p-4 space-y-3">
                            ...
                            <div x-data="{ showOverrides: Object.keys(segment.platform_contents || {}).length > 0 }">
                                ...
                                <div x-show="showOverrides" x-collapse class="mt-2 space-y-2">
                                    ...
                                    <div>
                                        <label class="flex items-center gap-1.5 text-xs font-medium text-gray-500 mb-1">
                                            <x-platform-icon platform="bluesky" size="xs" />
                                            Bluesky (300 car.)
                                        </label>
        ...
        {{-- Section 4c: Incrustation titre/sous-titre sur la 1ere image --}}
        <template x-if="segments.length > 0 && hasAnyMedia">
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 lg:p-8">
        ...
        function threadForm() {
            return {
                ...
                openStockLibrary(index) {
                    ...
                    // Pré-remplit avec les photo_keywords du LLM si disponibles, sinon le titre.
                ...
                selectStockPhoto(item) {
                    const seg = this.segments[this.activeMediaSegmentIndex];
                    if (!seg.media) seg.media = [];
                    // Remplace toute photo existante par celle du stock (un seul stock par segment).
                    seg.media = [{
                        type: 'image',
                        url: item.url_full,
                ...
                async regenerateSegment(index) {
                    ...
                    try {
                        ...
                        if (data.success && data.data) {
                            ...
                            segment.platform_contents.bluesky = data.data.bluesky || '';
                        }
                    } catch (err) {
                        // Silent fail
                    }
